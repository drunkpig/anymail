...
class EmailHtmlDisplay(Resource):
...
class WebMessageRouter(Resource):
    ...
    def getChild(self, name, request):
        if name:
            ...
            #return WebMessageDisplay(name, self.storage)  # email地址
            return EmailHtmlDisplay(name, self.storage)
        else:
            return WebMessageRootDisplay(name, self.storage) # 首页
